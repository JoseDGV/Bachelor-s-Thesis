...
//on the 2D Efficiency using the Efficiency spectrum histograms calculated
//on Eff2D.C
//
//To run, type on console:
//.L Eff2D_FineBinningW.C
//Acceptance t
//t.Loop()
///////////////////////////////////////////////////////////////////////////
...
void Efficiency::Loop()
{
  ...
  TH1F* hGenUpsilonEffNum = new TH1F ("hGenUpsilonEffNum", "Efficiency Numerator",PtnBins, PtBins); hGenUpsilonEffNum->Sumw2();
  //Defining Corrected Efficiency Numerator Histogram
  TH1F* hGenUpsilonEffNumCorr = new TH1F ("hGenUpsilonEffNumCorr", "Efficiency Corrected Numerator",PtnBins, PtBins); hGenUpsilonEffNumCorr->Sumw2();
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++)//Loop on ALL events
      ...
      for(int iQQ=0; iQQ < Gen_QQ_size; iQQ++)//Loop on generated upsilon
	{
	 ...
	 //Check for denominator conditions
	 ...
	 TLorentzVector *RecoQQ_MuPos = (TLorentzVector*) Reco_mu_4mom->At(iMuPos);
	  ...
	  Int_t BinNumAcc = hGenUpsilonAcc->FindBin(GenUpsilonPt,GenUpsilonRapidity);
	  Int_t BinNumAccW = hGenUpsilonAccW->FindBin(GenUpsilonPt,GenUpsilonRapidity);
	  Int_t BinNumEff = hGenUpsilonEff->FindBin(GenUpsilonPt,GenUpsilonRapidity);
	  Int_t BinNumEffW = hGenUpsilonEffW->FindBin(GenUpsilonPt,GenUpsilonRapidity);
	  ...
	  hGenUpsilonEffNum->Fill(GenUpsilonPt,AccVal*EffVal);
	  hGenUpsilonEffNumCorr->Fill(GenUpsilonPt,AccValW*EffValW);

	}
    }

  //Calculate ratio between histograms by dividing histograms
  TH1F *hGenUpsilonEffNumRatio  = new TH1F(*hGenUpsilonEffNumCorr); hGenUpsilonEffNumRatio->Divide(hGenUpsilonEffNum);

  //Set histograms parameters and write them out on root file
  TFile *fOut = new TFile("OutEff2D_FineBinningW.root","RECREATE");

  
  hGenUpsilonEffNumRatio->SetStats(0);
  hGenUpsilonEffNumRatio->SetTitle("Efficiency 2D Weighted Fine Binning");
  hGenUpsilonEffNumRatio->SetName("hGenUpsilonEffNumRatio ");
  hGenUpsilonEffNumRatio->SetTitleSize(1);
  hGenUpsilonEffNumRatio->SetLineColor(kBlue);
  hGenUpsilonEffNumRatio->SetLineWidth(3);
  hGenUpsilonEffNumRatio->GetYaxis()->SetRangeUser(0.96,1.04);
  hGenUpsilonEffNumRatio->GetYaxis()->SetTitleSize(0.05);
  hGenUpsilonEffNumRatio->GetYaxis()->SetTitleOffset(1);
  hGenUpsilonEffNumRatio->GetYaxis()->SetLabelSize(0.035);
  hGenUpsilonEffNumRatio->GetYaxis()->SetTitle("Ratio");
  hGenUpsilonEffNumRatio->GetXaxis()->SetTitle("Upsilon pT (GeV)");
  hGenUpsilonEffNumRatio->GetXaxis()->SetTitleSize(0.05);
  hGenUpsilonEffNumRatio->GetXaxis()->SetTitleOffset(1);
  hGenUpsilonEffNumRatio->GetXaxis()->SetLabelSize(0.035);
   
  hGenUpsilonEffNumRatio ->Write();
  ...
  hGenUpsilonEffNumRatio->SetStats(0);
  hGenUpsilonEffNumRatio->SetTitle("Efficiency 2D Weighted Fine Binning 3");
  hGenUpsilonEffNumRatio->SetTitleSize(1);
  hGenUpsilonEffNumRatio->SetLineColor(kBlue);
  hGenUpsilonEffNumRatio->SetLineWidth(3);
  hGenUpsilonEffNumRatio->GetYaxis()->SetRangeUser(0.96,1.04);
